...
def process(req):
    ...
    if not isinstance(call_dict.get('call'), int): # Checks if 'call' is a key in the request JSON, also checks if the value is an integer
        return "oooMalformed JSON received, send request again"

    if not len(call_dict.keys()) - 1 == call_dict.get('call'): # Checks if the number of functions declared equals the number of keys - 1 in the JSON
